...
const NavBar = ({ user, userProfile, handleLogout, history }) => {
  return (
    <>
      {user ? (
        <nav className="relative px-4 py-4 flex sm:flex-col justify-between items-center flex items-center justify-between flex-wrap bg-white py-4 lg:px-12 shadow border-solid border-t-2 border-blue-700 mb-4">
           <div className="flex justify-between lg:w-auto w-full lg:border-b-0 pl-6 pr-2 border-solid border-b-2 border-gray-300 pb-5 lg:pb-0">
            <div className="flex items-center flex-shrink-0 text-gray-800 mr-16"></div>
          <Link to="/">
            <img
              src="https://i.imgur.com/ZnXPhEq.jpg"
							alt="logo"
              className="justify-self rounded-full h-20 w-20 ring-4 ring-blue hover:opacity-75 my-3"
            />
          </Link>
</div>
          ...
          <Link to="" onClick={handleLogout}>
                <button className="text-white border border-blue-400 text-black-500 bg-blue-600 block rounded-md font-bold py-2 px-6 my-2 mx-4 flex items-center hover:bg-blue-500 hover:text-white">
                  Logout
                </button>
              </Link>
              </li>
